...
managed local Speaches example, see `deploy/transcription/speaches`. That
compose example binds the provider to `127.0.0.1:7322` and uses a
Docker-managed model cache volume. ACP Web UI does not install, start, stop,
supervise, require, or update that service.
...
both Chinese and English so the provider can auto-detect the language. Set it to
