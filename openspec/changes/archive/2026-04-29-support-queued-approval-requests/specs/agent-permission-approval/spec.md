...
**Reason**: Codex can emit multiple permission requests in one turn before the user resolves the first one. Treating the second request as unsupported can leave the local session stuck in `running`.

**Migration**: Store multiple pending permission requests for a session and expose them as an ordered approval queue with one active request at a time.
