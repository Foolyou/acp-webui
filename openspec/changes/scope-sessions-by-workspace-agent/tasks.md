...
- [ ] 1.1 Add SQLite migration fields for session title, native updated timestamp, import source metadata, and idempotent `(agent_id, external_session_id)` native imports.
- [ ] 1.2 Add storage models and helpers for workspace-agent session listing and native session import/update.
...
- [ ] 3.1 Add `GET /api/workspaces/{workspaceId}/agents/{agentId}/sessions` with runtime sync and workspace-agent filtering.
- [ ] 3.2 Update session creation and response helpers so new sessions and imported sessions preserve selected workspace-agent route context.
...
- [ ] 4.1 Add current-agent navigation state stored separately from last session creation profile state.
...
- [ ] 5.1 Add an agent switcher to the workspace Sessions surface and load the selected agent's list on change.
