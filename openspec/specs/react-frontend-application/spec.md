...
- **WHEN** the user submits a non-empty prompt while the visible session is running without pending approval
...
#### Scenario: User cannot queue prompt during approval
- **WHEN** the visible session is waiting for permission approval
- **THEN** the React frontend SHALL keep the composer visible but disable prompt submission
- **AND** it SHALL NOT call the prompt API for new prompts until approval is resolved
