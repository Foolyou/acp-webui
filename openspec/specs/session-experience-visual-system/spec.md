...
The frontend SHALL render tool call timeline items as compact rows by default.

#### Scenario: Tool call appears in timeline
- **WHEN** a tool call timeline item is available
- **THEN** the app SHALL render a thin row with tool kind or name, compact summary, and status
- **AND** it SHALL avoid showing raw JSON by default

#### Scenario: User expands tool call
- **WHEN** the user expands a tool call row
- **THEN** the app SHALL show available parameters, output snippets, raw payload access, and linked review artifacts if present
