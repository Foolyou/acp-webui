...
- **THEN** the browser or backend SHALL reject the prompt until approval is resolved
- **AND** no ACP prompt request SHALL be sent
- **AND** no queued prompt SHALL be created
...
- **WHEN** the browser loads a workspace session list route or compatibility agent-filtered route
...
The system SHALL enqueue follow-up prompts only when a session has a real active turn that is not blocked by pending approval.

#### Scenario: Follow-up submitted during active turn
- **WHEN** the user submits a prompt while active-turn metadata exists for the session and no permission approval is pending
